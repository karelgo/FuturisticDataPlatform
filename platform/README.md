...
docker build -t carina:0.2.0 . && k3d image import carina:0.2.0 -c carina

# Option A — plain Helm (no GitOps):
helm install carina charts/carina --set image.repository=carina --set image.tag=0.2.0
...
|---|---|
| KEEL | `planes/data-plane/` (Lakekeeper + CloudNativePG), `planes/compute-plane/` (Trino behind the lane router), `planes/trust-plane/` (Keycloak, OpenFGA, OPA loading the compiled `policy.rego` artifacts) |
