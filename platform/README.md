...
docker build -t carina:0.4.0 . && k3d image import carina:0.4.0 -c carina

# Option A — plain Helm (no GitOps):
helm install carina charts/carina --set image.repository=carina --set image.tag=0.4.0
...
|---|---|
| KEEL | ✅ [`planes/data-plane/`](planes/data-plane/) (Lakekeeper + CloudNativePG + Garage, Tier-0 backups) · ✅ [`planes/trust-plane/`](planes/trust-plane/) (Keycloak realm-as-code, OPA serving the generated `policies/` bundle, OpenFGA) · ⬜ `planes/compute-plane/` (Trino behind the lane router) |
